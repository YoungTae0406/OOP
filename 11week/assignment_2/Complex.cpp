...
Complex::Complex(float _val1, float _val2) {
     ...
     */
	set_val(std::make_pair(_val1, _val2));
}
...
Number *Complex::add(Number * num) {
	...
	temp->set_val(this->val().first+temp->val());
	delete(num);
	return temp;	
	}
	...
	return temp;
		...
		temp->set_val(this->val().first + temp->val().first, this->val().second +  temp->val().second);
		delete(this);
		delete(num);
		return temp;
	}
...
Number *Complex::mul(Number * num) {
	...
	return temp;
		...
		Complex* temp =dynamic_cast<Complex*>(num);
		temp->set_val(this->val().first * temp->val().first - this->val().second +  temp->val().second, this->val().first *
				temp->val().second + this->val().second * temp->val().first);
		delete(this);
		delete(num);
		return temp;
	}
...
void Complex::set_val(std::pair<double, double> val) {
     ...
     */
	value = std::make_pair (val.first, val.second);
}
